## List Revisions
GET /wp/v2/revisions/<parent>

Retrieve a paginated list of revisions for a specific post.

## Example Request
```bash
curl -X GET https://example.com/wp-json/wp/v2/revisions/42 \
...
| orderby | string | Sort collection by object attribute. Options: `date`, `id`, `include`, `relevance`, `slug`, `modified`. Default: `date`. |
...
| rest_forbidden | You do not have permission to list revisions. |
