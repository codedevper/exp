...
- **CertificateID** (number, max 10, required): Unique identifier of SSL certificate to be reissued
- **CSR** (string, max 32767, required): Certificate Signing Request (CSR)
- **AdminEmailAddress** (string, max 255, optional): Email address to send signed SSL certificate file to. Note: For Sectigo certificates (e.g. PositiveSSL) it is not possible to change AdminEmailAddress in reissue due to limitation from Certificate Authority. SSL file will be sent to AdminEmailAddress used in the initial activate call. For SSL.com certificates (e.g. Standard SSL), email can be changed. Certificate files are emailed by Namecheap. To avoid Namecheap communication being sent to your customer, omit this field and use getInfo. If omitted, SSL can only be obtained by calling getInfo.
...
- **AdminCountry** (string, max 2, optional*): Organization country name to be validated by Comodo validation staff and encoded into the issued certificate. * Not required for OV, required for EV. Format: Two-letter ISO 3166-1 alpha-2 country code. Restrictions apply. Cannot be changed after 45 days since first certificate issuance - altered values will be ignored and details from the latest issued certificate used.
...
Organization name and some organization details cannot be changed through reissue if reissue is made on/after 45 days since the initial order issuance. Altered values will be ignored and values encoded in the latest issued certificate will be used, reissue call will go through, but a \<Warning\> will be added into the API response. Parameters affected: AdminOrganizationName, AdminCountry, AdminStateProvince, AdminCity, AdminAddress1, AdminPostalCode

### EV only

These parameters are common only to Extended Validation (EV) certificate types.

- **CompanyIncorporationCountry** (string, max 2, optional): Country where the organization's legal existence was established (e.g., where it was incorporated). Format: Two-letter ISO 3166-1 alpha-2 country code.
...
Some organization details cannot be changed through reissue if reissue is made on/after 45 days since the initial order issuance. Altered values will be ignored and values encoded in the latest issued certificate will be used, reissue call will go through, but a \<Warning\> will be added into the API response. Parameters affected: CompanyIncorporationCountry, CompanyIncorporationStateProvince, CompanyIncorporationLocality, CompanyIncorporationDate, CompanyDBA
...
Reissue success will result in child certificate creation in status "PURCHASED"

Reissue completion will result in parent certificate status change to "REPLACED", and child status change to "ACTIVE"

Reissue failure caused by an error from Namecheap will not create a child certificate. Reissue parameters can be corrected and activation retried at once

Reissue failure caused by an error from Certificate Provider will change the certificate's status to "PURCHASEERROR". Contact support to have the status reset and retry reissue
...
- **ID**: ID of certificate that was reissued
- **IsSuccess**: Indicates whether the SSL certificate was reissued successfully
...
- **4011331**: SSL Reissue is already in progress.
- **4035900**: Cannot create certificate for reissue
- **3027295**: We are unable to process your order. Error Details: %1$s
- **4080167**: 3 yrs SSL are discontinued due to security reasons https://www.namecheap.com/support/knowledgebase/article.aspx/9511/38/. SSL will be reissued for 2 yrs. Reissue again before SSL expiration to prolong SSL for extra months you can't currently use. Extra months are not refundable for expired certificates.
...
- **3011296**: The CSR provided is invalid to generate a wildcard certificate. Please make sure that you generate a CSR with a Common Name (CN) that starts with '*.' (ex: *.yourdomain.com)
