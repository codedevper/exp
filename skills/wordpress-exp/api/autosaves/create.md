## Create an Autosave
POST /wp/v2/autosaves/<parent>

Create an autosave for a specific post. Autosaves are automatically created by the block editor but can also be created via the REST API.

## Example Request
```bash
curl -X POST https://example.com/wp-json/wp/v2/autosaves/42 \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -d '{
    "title": "My Post (Autosave)",
    "content": "<!-- wp:paragraph --><p>Autosaved content.</p><!-- /wp:paragraph -->"
  }'
```
...
| title | string | The title of the autosave. |
| content | string | The content of the autosave. |
| excerpt | string | The excerpt of the autosave. |
...
| rest_forbidden | You do not have permission to create autosaves. |
