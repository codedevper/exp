...
Update an installed plugin's status (activate/deactivate). Requires authentication with activate_plugins capability.

## Example Request
```bash
curl -X PUT https://example.com/wp-json/wp/v2/plugins/akismet%2Fakismet.php \
...
| status | string | New activation status. Options: active, inactive |
| context | string | Scope under which the request is made. Use edit for full response. Options: view, edit. Default: view |
...
| rest_forbidden | You do not have permission to activate/deactivate plugins. |
